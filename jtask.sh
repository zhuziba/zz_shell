...
usage () {
    ...
    echo -e "$cmd_jtask <js_name> now    # 依次执行，无论是否设置了随机延迟，均立即运行，前台会输出日志，同时记录在日志文件中，EnableTaskFinishShell＝true时将额外运行task_finish.sh"
    ...
    echo -e "$cmd_otask <js_path> now    # 依次执行，无论是否设置了随机延迟，均立即运行，前台会输出日志，同时记录在日志文件中，EnableTaskFinishShell＝true时将额外运行task_finish.sh"
...
find_file_and_path () {
    ...
    for path in $seek_path; do
        if [ -f $path/$file_name_tmp1.js ]; then
            ...
            which_path=$path
            break
        elif [ -f $path/$file_name_tmpjs ]; then
